...
設定画面の「教材パック」から、`pack.json` を含む外部教材フォルダーをインポートできます。

教材パックは配布教材、文章ルール、練習モード、例文を扱う読み取り用データです。利用者が保存した修正例や好みは、教材パック本体ではなく「学習セット」に保存します。

会社用・個人用の非公開教材は、GitHubに上げない隔離エリア `private-content/` に置いてください。

詳しい作り方と注意点は [教材パックのインポートガイド](docs/study-pack-import-guide.ja.md) を参照してください。
