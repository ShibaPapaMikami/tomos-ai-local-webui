...
- Standard Markdown Specs: `AGENTS.md`、`DESIGN.md`、`VOICE.md`、`MEMORY.md`、`PLUGIN.md` をTOMOS標準仕様として扱い、AIエージェント、プラグイン、教材パック、キャラクター体験の一貫性を保つ。

### Standard Markdown Specs

TOMOSでは、AIエージェントが実装、UI、会話、記憶、拡張仕様を同じ前提で読めるように、以下のMarkdownを標準仕様にする。

```text
AGENTS.md
= 実装ルール、作業境界、安全ルール、検証ルール

DESIGN.md
= UIデザインルール、色、余白、フォント、ボタン、カード、レスポンシブ

VOICE.md
= キャラクター、口調、敬語、呼び方、話速、相槌、NG表現

MEMORY.md
= 記憶、人物、学習情報、保存対象、忘れる条件、スコープ

PLUGIN.md
= プラグイン仕様、権限、設定、UI導線、参照できるデータ範囲
```

採用方針:

- TOMOS本体は5ファイルを必須仕様として持つ。
- 公式プラグインは `PLUGIN.md` と `DESIGN.md` を必須にする。
- キャラクター、返信支援、音声、アバターを持つプラグインは `VOICE.md` も必須にする。
- 人物メモ、教材パック、記憶、資料検索を扱うプラグインは `MEMORY.md` も必須にする。
- 実装をAIエージェントへ依頼するリポジトリやパッケージは `AGENTS.md` を必須にする。
- 5ファイルはアプリが読み込む設定ファイルではなく、まずは人間とAIエージェントの共通仕様書として扱う。
- 将来はプラグイン審査、AI生成UI、キャラクター再現性、記憶スコープ確認に利用する。

必須化ロードマップ:

- Phase 1: TOMOS本体に `AGENTS.md` と `DESIGN.md` を整備する。
- Phase 2: プラグイン制作テンプレートに `PLUGIN.md` と `DESIGN.md` を追加する。
- Phase 3: キャラクター機能に `VOICE.md` を接続し、マイキャラ、アバター、返信支援の話し方を統一する。
- Phase 4: Local Context Coreに合わせて `MEMORY.md` を整備し、保存、参照、削除、スコープのルールを明文化する。
- Phase 5: プラグインストアで5ファイルの有無を検査し、足りない場合は公開前の警告を出す。

やらないこと:

- 既存プラグインを即時停止しない。
- 5ファイルを最初から機械実行形式にしない。
- 他社ブランドの `DESIGN.md` をそのままTOMOSの見た目として使わない。
- `MEMORY.md` に個人情報や秘密情報を直接書かない。
