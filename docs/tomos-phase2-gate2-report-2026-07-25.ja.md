...
コード、回帰テスト、localhost安全境界、ブラウザー表示は合格した。TOMOSアプリ版ではマイク許可、実マイクの列挙、会話欄での録音開始・停止、停止3秒後まで遅延結果が追加されないことまで合格した。再診断により、当初の0%表示は入力メーター開始前の値であり、WebViewには環境音とシステム音が届くことを確認した。音を認識しているか分かる5段階インジケーターも追加し、無音時のレベル0と環境音入力時のレベル1をアプリ版で確認した。強いシステム音ではVADが発話終了を検出し、ASR要求を1回だけ実行した。一方、人の声による発話検出から650ms後の文字起こし確定までは画面上で確認できていない。Gate 2は`検証中`を維持し、Phase 3には進まない。
...
Mac実機: 一部合格（WebViewへの音声入力、5段階表示、システム音によるVAD確定。人の声は未確認）
PC幅: 合格（1440×900、1280×820、960×640）
スマホ幅: 合格（390×844）
既存3経路: 自動テスト合格（Nemotron、Whisper CLI、ブラウザーSpeechRecognition）、実音声は未確認
保存・外部送信: 0件（音声永続保存なし、localhost以外なし、自動送信なし）
未完了: 人の声による発話検出、会話入力欄への文字起こし、650ms後の1回確定
```
...
- 日本語合成音声は調整後も発話として確定しなかったため、人の声による最終確認は残る
...
自動テストはすべて合格。実機は録音開始、VADによる自動停止、ASR要求、停止後破棄まで合格し、人の声による会話入力欄への確定確認が残る。`scripts/test_server_helpers.py` はlocalhost一時ポートを使うため権限付きで実行した。
...
閾値調整後のTauriアプリbundleで、次を確認する。

1. 無音では文字起こしが始まらない。
2. 日本語で発話すると状態が「待機」から「発話検出」「確定中」へ変わる。
3. 発話終了から約650ms後に入力欄へ1回だけ確定する。
4. Nemotron、Whisper、ブラウザーSpeechRecognitionの選択経路を各1回確認する。

合格後にGate 2を`合格`へ更新し、Phase 3の開始承認を待つ。
