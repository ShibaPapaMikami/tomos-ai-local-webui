...
assert.match(indexHtml, /src="\/i18n\.js\?v=0\.8\.206-tomos4"/);
assert.match(indexHtml, /href="\/styles\.css\?v=0\.8\.206-tomos4"/);
...
const characterMenuIndex = indexHtml.indexOf('id="character-toggle"');
const contextMemoryMenuIndex = indexHtml.indexOf('id="context-memory-toggle"');
const languageModelsMenuIndex = indexHtml.indexOf('id="language-models-toggle"');
const extensionsGroupIndex = indexHtml.indexOf('data-i18n="management.extensions"');
assert.ok(characterMenuIndex < contextMemoryMenuIndex, "long-term memory should be next to My Character");
assert.ok(contextMemoryMenuIndex < languageModelsMenuIndex, "long-term memory should appear before language models");
assert.ok(contextMemoryMenuIndex < extensionsGroupIndex, "long-term memory should not be inside Extensions");
