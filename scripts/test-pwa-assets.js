...
assert.match(index, new RegExp(`href="/styles\\.css\\?v=${DOWNLOAD_PROGRESS_ASSET_VERSION}"`));
...
assert.match(index, new RegExp(`src="/management\\.js\\?v=${DOWNLOAD_PROGRESS_ASSET_VERSION}"`));
assert.match(index, new RegExp(`src="/download-progress\\.js\\?v=${DOWNLOAD_PROGRESS_ASSET_VERSION}"`));
assert.match(index, /src="\/workspace\.js\?v=0\.8\.225-note-no-save"/);
assert.match(index, new RegExp(`src="/app\\.js\\?v=${DOWNLOAD_PROGRESS_ASSET_VERSION}"`));
...
assert.match(sw, new RegExp(`const CACHE_NAME = "gemma4-pwa-${DOWNLOAD_PROGRESS_ASSET_VERSION}"`));
...
assert.match(mobileHtml, new RegExp(`src="/pwa\\.js\\?v=${DOWNLOAD_PROGRESS_ASSET_VERSION}"`));
