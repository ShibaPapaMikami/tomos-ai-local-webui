...
def test_should_read_search_result_pages_detects_complete_list_request() -> None:
    ...
    assert server.should_read_search_result_pages("全て教えて")
    assert server.should_read_search_result_pages("候補、全作品を出して")
    ...
    assert server.should_read_search_result_pages("リストアップして")
    ...
    assert not server.should_read_search_result_pages("リストラについて教えて")
    assert not server.should_read_search_result_pages("安全性について教えて")
    assert not server.should_read_search_result_pages("安全部門について教えて")
    assert not server.should_read_search_result_pages("完全な状態を説明して")
    assert not server.should_read_search_result_pages("完全シリーズについて教えて")
