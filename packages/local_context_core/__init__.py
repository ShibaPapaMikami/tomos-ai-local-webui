...
def remember(item: dict[str, object], *, scope: dict[str, object] | None = None) -> dict[str, object]:
    ...
    record = ContextRecord(
        id=explicit_id or stable_record_id("memory", scope_type, scope_id, owner_type, owner_id, memory_type, source_type, source_id, text),
        text=text,
        snippet=text,
        sourceType="memory",
        sourceId=source_id,
...
def update_context_record(db_path: Path, record_id: str, updates: dict[str, object]) -> dict[str, object]:
    ...
    with connect(db_path) as connection:
        ...
        sensitivity = str(updates.get("sensitivity") or metadata.get("sensitivity") or "").strip()
        if contains_sensitive_text(text) and sensitivity != "protected":
            return {
                "ok": False,
                "needsReview": True,
                "reason": "センシティブ情報の可能性があるため自動保存しません。",
        ...
        metadata["sensitivity"] = sensitivity or "normal"
